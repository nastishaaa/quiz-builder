import type { Request, Response } from 'express';
import { getAllQuizes, getQuizById, createQuize, deleteQuize } from '../services/index.js';
import createHttpError from 'http-errors';
import type { CreateQuizInput, Quiz, QuizResponse } from '../types/index.js';

export async function getAllQuizesController(req: Request, res: Response) {
    try {
        const result = await getAllQuizes();
        res.status(200).json({
            status: 200,
            message: "Quizes fetched successfully",
            data: result
        });
    } catch (error) {
        console.log(error);
        res.status(500).json({
            status: 500,
            message: 'Something went wrong!',
        });
    }
}

export async function getQuizByIdController(req: Request, res: Response) {
    try {
        const id = Number(req.params.id);

        if (isNaN(id)) {
            throw createHttpError(400, 'Invalid train id');
        }

        const result: QuizResponse = await getQuizById(id);      

        res.status(200).json({
            status: 200,
            message: "Quiz fetched successfully",
            data: result
        });
    } catch (error: unknown) {
        res.status(404).json({
            status: 404,
            message: 'Something went wrong! There are no quiz with your id.',
        });
    }
}

export async function postQuizeController(req: Request, res: Response) {
    try {
    const payload: CreateQuizInput = req.body;

    const quiz = await createQuize(payload);

    res.status(201).json({
        status: 201,
        message: "Quiz created successfully",
        data: quiz
        });
    } catch (error) {
        res.status(500).json({
            status: 500,
            message: 'Something went wrong!',
        });
    }
}

export async function deleteQuizeController(req: Request<{ id: string }>, res: Response) {
    try {
        await deleteQuize(+req.params.id);
        res.status(204).send();
    } catch (err: any) {
        console.error("FULL ERROR:", err);
        res.status(err.status || 500).json({ status: err.status || 500, message: err.message });
    }
}